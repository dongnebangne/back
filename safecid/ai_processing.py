...
# def process_image_with_sam_and_lama_or_sd(img, point_coords, point_labels, dilate_kernel_size, sam_model_type, sam_ckpt, lama_config, lama_ckpt, text_prompt=None, device="cuda"):
#     # SAM을 통해 마스크 생성
...
#     if not text_prompt:
#         img_inpainted = inpaint_img_with_lama(img, masks[0], lama_config, lama_ckpt, device=device)
#     # 프롬프트가 있는 경우 Stable Diffusion으로 인페인팅 (fill anything)
#     else:
#         img_inpainted = fill_img_with_sd(img, masks[0], text_prompt, device=device)
#
#     # 인페인팅된 이미지 저장
#     inpainted_image_path = output_dir / 'inpainted_image.png'
#     save_array_to_img(img_inpainted, inpainted_image_path)
#
#     return inpainted_image_path, mask_file_paths
...
#     # 프롬프트가 있는 경우 Stable Diffusion으로 인페인팅 (fill anything)
#     else:
#         img_inpainted = fill_img_with_sd(img, masks[2], text_prompt, device=device)
#
#     # 인페인팅된 이미지 저장
#     inpainted_image_path = output_dir / 'inpainted_image.png'
#     save_array_to_img(img_inpainted, inpainted_image_path)
